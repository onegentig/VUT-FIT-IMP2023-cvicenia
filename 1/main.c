...
/*                                                                                  */
/*  Reseni vytvoril a odevzdava: Kramec Filip, xkrame00 */
/*                                                                                  */
/************************************************************************************/
...
unsigned char corrl[9] = "xonege99";  // Vas login
...
/* Inicializace UART - nastaveni prenosove rychlosti 115200Bd, 8 bitu, bez
 * parity */
void UART0Init(void) {
     ...
     UART0->C2 |= (UART0_C2_TE_MASK
                   | UART0_C2_RE_MASK);  // Zapnout vysilac i prijimac
}

/* vyslani jednoho znaku (ch) pres UART - funkce vycka, az je vysilaci buffer
 * prazdny, pak posle */
void SendCh(char ch) {
...
/* prijeti jednoho znaku pres UART - funkce ceka na prichozi znak a ten vrati
 * jako vysledek */
char ReceiveCh(void) {
	// TODO: Funkci doplnte!
...
int main(void) {
     ...
     PinInit(); /* Inicializace vstupu a vystupu potrebnych pro tuto ulohu -
                   piny RX a TX UART0 */

     UART0Init(); /* Inicializace modulu UART0 - nastaveni prenosove rychlosti,
                     parametru prenosu */

     // TODO: Doplnte dle popisu
     while (1) {
